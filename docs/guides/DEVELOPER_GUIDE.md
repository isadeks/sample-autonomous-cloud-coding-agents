...
An empty result means the stack predates the tag; redeploy to surface it. Note this is the stack-wide default — individual repos can still override the backend per repo via the Blueprint `compute_type` field (see [Repo onboarding](../design/REPO_ONBOARDING.md)).
