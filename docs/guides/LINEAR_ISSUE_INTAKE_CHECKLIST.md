...
> **How ABCA reads the issue:** the issue **title + description** become the task description; comments add context (oldest are trimmed first if the combined content is large). For writing effective descriptions in depth, see the [Prompt guide](./PROMPT_GUIDE.md). For label mechanics (`bgagent`, `:decompose`, `:auto`, `:help`) and the trigger flow, see the [Linear setup guide](./LINEAR_SETUP_GUIDE.md).
...
- [ ] A turn or budget expectation is set if the work is unusually large or small (`--max-turns` / `--max-budget` at submit time; see the [Prompt guide](./PROMPT_GUIDE.md#calibrating---max-turns)).
...
- [ ] Prerequisite work is merged, or this issue is modeled as a **sub-issue** with the correct `blocked by` relation so orchestration runs it in order (see [parent/sub-issue orchestration](./LINEAR_SETUP_GUIDE.md#parentsub-issue-orchestration)).
...
- [Prompt guide](./PROMPT_GUIDE.md) — writing effective task descriptions in depth.
- [Linear setup guide](./LINEAR_SETUP_GUIDE.md) — trigger labels, the approval flow, and orchestration.
- [User guide](./USER_GUIDE.md#tips-for-being-a-good-citizen) — being a good citizen on a shared platform.
