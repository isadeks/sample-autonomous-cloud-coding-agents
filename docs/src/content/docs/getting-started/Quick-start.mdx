...
- **Trigger from Linear**: Apply a label to a Linear issue - see the [Linear integration setup guide](/sample-autonomous-cloud-coding-agents/using/linear-setup-guide), and the [recovery guide](/sample-autonomous-cloud-coding-agents/using/linear-recovery-guide) for reading status reactions and replying when ABCA needs input
