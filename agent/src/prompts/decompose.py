"""Workflow section for ``coding/decompose-v1`` — decomposition planning run as
a real agent task, rather than as a blind two-call model invocation inside the
webhook Lambda.

Unlike the other coding workflows this one does NOT change code or open a PR: it
clones the repo (so it plans with FULL repository context, which a planner
running inside the webhook Lambda never had — it lacked the repo, so it timed out
and planned blind), decides whether the issue should be decomposed, and if so
emits a structured decomposition-plan JSON as its final message. The platform's
existing write-back + seed machinery (idempotent issueCreate/issueRelationCreate)
consumes that plan and turns it into sub-issues; the agent does NOT create
sub-issues itself.
